...
/* getbits:  get n bits from position p (book pg 49) */
int getbits(mem_addr_t x, int p, int n)
{
	return (x >> (p+1-n)) & ~(~0 << n);
}
...
void accessData(mem_addr_t addr)
{
	...
	cache_line_t *line, *mru, *lru, *hit, *empty;
	hit = NULL;
	empty = NULL;

	line = &cache[getbits(addr,b+s-1,s)][0];
	mru = &cache[getbits(addr,b+s-1,s)][0];
	lru = &cache[getbits(addr,b+s-1,s)][0];

	for(int i = 0; i < E; i++)
	{
		line = &cache[getbits(addr,b+s-1,s)][i];
		if(line->valid == 1)
		{
			if(line->tag == tbits)
			{
		...
		if(line->counter > mru->counter)
			mru = line;
		if(line->counter < lru->counter)
			lru = line;
	}
	...
	if(NULL == empty)
	{
		...
		eviction_count++;
		lru->tag = tbits;
		lru->counter = mru->counter + 1;
	} else {
		// load
		empty->valid = 1;
		empty->tag = tbits;
		empty->counter = mru->counter + 1;
	}
