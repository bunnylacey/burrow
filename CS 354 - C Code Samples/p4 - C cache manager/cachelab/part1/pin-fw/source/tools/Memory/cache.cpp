				  ...
				  const unsigned long block_size)
{
	...
	{
		printf("ERROR: Cache block size (%ld) is not a power of two.\n",block_size);
		return NULL;
	}
...
static void determine_address_widths(struct cache* c)
{
	...
	if (offset_width == -1)
	{
		printf("ERROR: Cache block size (%ld) must be a power of two.\n",c->block_size);
	} else c->addr_info.offset_width = offset_width;
	if (index_width == -1)
	{
		printf("ERROR: Number of sets in cache (%ld) must be a power of two.\n", c->number_of_sets);
