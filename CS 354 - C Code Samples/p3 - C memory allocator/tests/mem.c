...
void* Mem_Alloc(int size)
{
	...
	block_header *current = NULL, *split = NULL, *bestfit = NULL;
	current = list_head;
	while(NULL != current)
	{
		if(isFree(current))
		{
			if(nsize <= current->size_status)
			{
				if(NULL == bestfit)
				{
					...
					if(current->size_status < bestfit->size_status)
					{
						bestfit = current;
					}
				}
	...
	if(bestfit->size_status - nsize >= (int)sizeof(block_header) + 4)
	{
		split = (block_header *)((char*)bestfit + (int)sizeof(block_header) + nsize);
		split->next = bestfit->next;
		bestfit->next = split;
		bestfit->next->size_status = bestfit->size_status - nsize - (int)sizeof(block_header);
		bestfit->size_status = nsize;
	}

	// Mark the allocated block and return it
	setAllocated(bestfit);
	return (void *)(bestfit + (int)sizeof(block_header));
}
