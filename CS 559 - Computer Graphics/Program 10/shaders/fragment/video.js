...
script.text = `
    ...
    void main(void)
    {
        ...
        vec3 tNormal = normalize(uNormal * vec4(fNormal, 0.0)).xyz;
    
        // create flat shading but offset slightly by vertex normals so it isn't 100% flat;
        vec3 facet_normal = normalize((tNormal*0.00008) + cross(dFdx(tPosition), dFdy(tPosition)));

        vec3 ambient_color  = ambient_coeff  * fColor;
        vec3 specular_color = specular_coeff * pow(max(0.0,dot(facet_normal,dir)),specular_exp) * light_color;

        vec3 diffuse_color = (max(.2,dot(facet_normal,dir)) * texColor.xyz);
